...
function isFolderMatch(folderPath, targetFolder) {
    const normalizedFolder = (0, obsidian_1.normalizePath)(folderPath);
    const normalizedTarget = (0, obsidian_1.normalizePath)(targetFolder);
...
function getTemplatesFolder(app) {
    ...
    const options = templatesPlugin.instance?.options ?? templatesPlugin.options ?? null;
    if (!options) {
        return null;
    }
    const candidates = [
...
async function moveFile(app, file, targetPath) {
    ...
    if (existingFile && existingFile instanceof obsidian_1.TFile) {
